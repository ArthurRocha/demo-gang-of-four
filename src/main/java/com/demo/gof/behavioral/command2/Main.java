...
public class Main {
	...
	public static void main(String[] args) {
		ClientCommandParameters clientCommandArgument = new ClientCommandParameters("{name:'Person', age:25}");
		System.out.println("Example 1");
		Command command = new DoSomethingCommandHandler(new ReceiverService(), clientCommandArgument);
		command.execute();
		
		System.out.println("Example 2");
		CommandManagerInvoker invoker = new CommandManagerInvoker();
		ReceiverService receiver = new ReceiverService();
		invoker.addCommand(new DoSomethingCommandHandler(receiver, clientCommandArgument));
		invoker.addCommand(new DoAnotherthingCommandHandler(receiver, clientCommandArgument));
		invoker.runCommands();
